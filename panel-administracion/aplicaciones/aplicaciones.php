...
// Objeto
$Tabla = new Aplicacion($Cnx, $xajax);
...
switch ($Acc) {
	case ACC_ALTA:
		...
		if ($mod_Contenido === true){
			if(isset($_POST['redirigir'])){
				$location = basename($_SERVER['SCRIPT_NAME']) . "?MOD=aplicaciones&REDIRIGIR=S&ACC=" . ACC_MODIFICACION . "&COD=" . $Tabla->Registro['id_aplicacion'];
			}
			else{
	...
	case ACC_MODIFICACION:
   		if (isset($_GET['COD'])){
			$mod_Contenido = $Tabla->Update($_GET['COD'], 'id_aplicacion');
			// Si grabo
			if ($mod_Contenido === true){
	...
	case ACC_BAJA:
   		if (isset($_GET['COD'])){
			$mod_Contenido = $Tabla->Delete($_GET['COD'], 'id_aplicacion');
			// Si borro
			if ($mod_Contenido === true){
				header("Location: ".basename($_SERVER['SCRIPT_NAME']).$ParamMod);
				exit();
