...
$mod_Contenido = '';
$error = "";
$html = new nyiHTML('productos/fotos-producto.htm');

// Si viene con POST
if($_SERVER['REQUEST_METHOD'] == "POST"){
	if(isset($_POST["subefoto"]) && $_POST["subefoto"] == _SI && is_uploaded_file($_FILES["fotonueva"]["tmp_name"])){
		$res = $objProducto->asociarNuevaFoto($_GET['COD'], $_FILES["fotonueva"]["tmp_name"], $_FILES["fotonueva"]["name"]);
		if($res != ""){
			$error = $res;
		}
	}
	elseif(isset($_POST["orden"]) && $_POST["orden"] != ""){
		...
		$res = $objProducto->ordenarFotos($_GET['COD'], $nuevoOrden);
		if($res != ""){
			$error = $res;
		}
		else{
...
$galeria = $objProducto->obtenerGaleriaFotos($_GET['COD']);
while(!$galeria->EOF){
	$extension = $galeria->fields['extension'];
	$nomSinExt = $galeria->fields['nombre_imagen']; 
	$html->append('FOTOS', array('nombre_imagen'=>$nomSinExt, 'url'=>DIR_HTTP_FOTOS_PRODUCTOS."{$_GET['COD']}/{$nomSinExt}.{$extension}?c=" . time()));
	$galeria->MoveNext();
}
...
$Cab->assign('NOMFORM', 'FOTOS DEL PRODUCTO: ' . strtoupper($objProducto->getNombre($_GET['COD'])));
...
$diagramacion = $objProducto->getDiagramacion($_GET['COD']);
switch($diagramacion){
	case 1:
...
$html->assign('LARGO_THUMBNAIL_PRODUCTO', $width);
$html->assign('ANCHO_THUMBNAIL_PRODUCTO', $height);
$html->assign('id_producto', $_GET['COD']);
$html->assign('ERROR', $error);
$xajax->setRequestURI(DIR_HTTP.'productos/fotos-producto-ajax.php');
