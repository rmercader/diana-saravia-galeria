...
include_once(DIR_BASE.'pedidos/pedido.class.php');
...
function modificarEstado($id, $estadoVal){
	...
	// Validaciones
	$idPedido = (int)$id;
	if(in_array($estadoVal, array('nuevo', 'enviado', 'completo'))){
		$estado = $estadoVal;
		$Cnx = nyiCNX(); // Creo la conexion
		$vta = new Pedido($Cnx, $xajax);
		$res = $vta->modificarEstado($idPedido, $estado);
		if($res == ""){ 
