...
function eliminarFoto($idResultado, $nombre){
	...
	$prn = new Resultado($Cnx);
	$prn->eliminarFoto($idResultado, $nombre);
	$galeria = $prn->obtenerGaleriaFotos($idResultado);
	$html = "";
	while(!$galeria->EOF){
		$extension = $galeria->fields['extension'];
		$nomSinExt = $galeria->fields['nombre_imagen']; 
		$url = DIR_HTTP_FOTOS_RESULTADOS."{$idResultado}/{$nomSinExt}.{$extension}?c=" . time();
